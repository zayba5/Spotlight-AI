...
if 'user_preferences' not in st.session_state:
    st.session_state.user_preferences = {
        ...
        'search_history': [
            {'query': 'coffee shops near me', 'timestamp': '2024-11-10 14:30:00', 'results': 5},
            {'query': 'italian restaurants', 'timestamp': '2024-11-09 19:15:00', 'results': 8},
            {'query': 'quiet study spots', 'timestamp': '2024-11-08 10:00:00', 'results': 3},
            {'query': 'brunch places downtown', 'timestamp': '2024-11-07 11:45:00', 'results': 6}
        ],
        'feedback_history': [
            {'place': 'Blue Bottle Coffee', 'feedback': 'liked', 'date': '2024-11-10'},
            {'place': 'The Grill', 'feedback': 'disliked', 'date': '2024-11-09'},
            {'place': 'Cafe Frascati', 'feedback': 'liked', 'date': '2024-11-08'}
        ]
    }
    ...
    st.session_state.saved_places = [
        {
            ...
            'price': '$$',
            'category': 'Coffee & Tea',
            'saved_date': '2024-11-10',
            'visit_count': 3,
            'notes': 'Great for working! Fast WiFi.'
        },
        {
            'name': 'Cafe Frascati',
            'rating': 4.3,
...
# Page Header
st.title("📚 Your History & Saved Places")
st.caption("Track your searches, save favorites, and get personalized recommendations")
...
# Main Tabs
tab1, tab2, tab3, tab4 = st.tabs(["❤️ Saved Places", "🔍 Search History", "👍 Feedback History", "🎯 Recommendations"])

with tab1:
    st.markdown("### ❤️ Your Saved Places")
    ...
    with col3:
        if st.button("📥 Export List"):
            st.download_button(
                "Download CSV",
                data=pd.DataFrame(st.session_state.saved_places).to_csv(index=False),
    ...
    if st.session_state.saved_places:
        for place in st.session_state.saved_places:
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                if st.button(f"🗺️ View on Map", key=f"map_{place['name']}"):
                    st.info(f"Opening map for {place['name']}")
            with col2:
                if st.button(f"✏️ Edit Notes", key=f"edit_{place['name']}"):
            ...
            with col4:
                if st.button(f"🗑️ Remove", key=f"remove_{place['name']}", type="secondary"):
...
with tab2:
    ...
    if st.session_state.user_preferences.get('search_history'):
        for idx, search in enumerate(st.session_state.user_preferences['search_history']):
            ...
            with col3:
                if st.button(f"🗑️", key=f"delete_search_{idx}"):
...
with tab3:
    ...
    if st.session_state.user_preferences.get('feedback_history'):
        for feedback in st.session_state.user_preferences['feedback_history']:
            ...
            with col1:
                if st.button(f"🔍 View Place Details", key=f"feedback_{feedback['place']}"):
                    st.info(f"Opening details for {feedback['place']}")
            with col2:
                if st.button(f"🗑️", key=f"delete_feedback_{feedback['place']}"):
